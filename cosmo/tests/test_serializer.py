...
def test_l2vpn_errors(capsys, mock_cosmo_config_fixture):
    ...
    vpws_incorrect_terminations["l2vpn_list"].append(
        {
            ...
            "type": "VPWS",
            "terminations": [
                {"__typename": "L2VPNTerminationType", "assigned_object": {}},
                {"__typename": "L2VPNTerminationType", "assigned_object": {}},
                {"__typename": "L2VPNTerminationType", "assigned_object": {}},
            ],
        }
    ...
    with pytest.raises(
        ...
        match=r"VPWS circuits are only allowed to have 2 terminations",
    ):
    ...
    unsupported_type_terminations["l2vpn_list"].append(
        {
            ...
            "type": "VPWS",
            "terminations": [
                {"__typename": "L2VPNTerminationType", "assigned_object": None},
                {"__typename": "L2VPNTerminationType", "assigned_object": {}},
            ],
        }
    )
    ...
    vpws_non_interface_term["l2vpn_list"].append(
        {
            ...
            "terminations": [
                {
                    "__typename": "L2VPNTerminationType",
                    "assigned_object": {"__typename": "VLANType"},
                },
                {
                    "__typename": "L2VPNTerminationType",
                    "assigned_object": {"__typename": "VLANType"},
...
def test_router_logical_interface(capsys):
    ...
    assert re.search(
        r"0.1] sub-interface number should be same as VLAN \(123\)", output.out
    )
    assert re.search(
...
def test_router_vrf_rib():
    ...
    assert (
        "L3VPN-TEST.inet.0"
        in sd["routing_instances"]["L3VPN-TEST"]["routing_options"]["rib"]
    )
    assert (
        "L3VPN-TEST.inet.0"
        in sd["routing_instances"]["L3VPN-TEST"]["routing_options"]["rib"]
    ...
    assert (
        rib["L3VPN-TEST.inet.0"]["static"]["10.114.23.36/32"]["next_hop"]
        == "10.30.0.154"
    )
    assert (
        rib["L3VPN-TEST.inet6.0"]["static"]["fd98::1/128"]["next_hop"] == "et-0/0/2.0"
    )
    assert (
        rib["L3VPN-TEST.inet.0"]["static"]["10.114.23.36/32"]["resolve_direct"] == True
    )
    assert rib["L3VPN-TEST.inet6.0"]["static"]["fd98::1/128"]["resolve_direct"] == True
...
def test_router_ips():
    ...
    mgtm_routing_instance_rib = sd["routing_instances"]["mgmt_junos"][
        "routing_options"
    ...
    assert unit_v4["families"]["inet"]["address"]["45.139.138.8/29"] == {
        "primary": True
    }
...
def test_router_case_local_l2x():
    ...
    assert (
        ...
        == l2c["interfaces"]["ifp-0/0/5.7"]["remote_label"]
    )
    assert (
        l2c["interfaces"]["ifp-0/0/5.7"]["local_label"]
        == l2c["interfaces"]["ifp-0/0/4.7"]["remote_label"]
    )
...
def test_router_case_local_bgpcpe():
    ...
    assert (
        not "export"
        in groups_L3VPN["CPE_ifp-0-1-2-4"]["family"]["ipv4_unicast"]["policy"]
    )
    assert (
        not "export"
        in groups_L3VPN["CPE_ifp-0-1-2-4"]["family"]["ipv6_unicast"]["policy"]
    )
...
def test_router_case_policer():
    ...
    assert d["interfaces"]["ae0"]["units"][101]["families"]["inet"]["policer"] == [
        "arp POLICER_IXP_ARP"
    ]
    assert d["interfaces"]["ae0"]["units"][101]["families"]["inet"]["filters"] == [
        "input-list [ EDGE_FILTER ]"
    ]
...
def test_router_case_sampling():
    [d] = get_router_sd_from_path("./test_case_sampling.yaml")
    ...
    assert "inet" in d["interfaces"]["ae0"]["units"][101]["families"]
    assert "inet6" in d["interfaces"]["ae0"]["units"][101]["families"]
    assert not "sampling" in d["interfaces"]["ae0"]["units"][101]["families"]["inet"]
    assert not "sampling" in d["interfaces"]["ae0"]["units"][101]["families"]["inet6"]

    assert 159 in d["interfaces"]["ae0"]["units"]
    assert "families" in d["interfaces"]["ae0"]["units"][159]
    assert "inet" in d["interfaces"]["ae0"]["units"][159]["families"]
    assert "inet6" in d["interfaces"]["ae0"]["units"][159]["families"]
    assert "sampling" in d["interfaces"]["ae0"]["units"][159]["families"]["inet"]
    ...
    assert "families" in d["interfaces"]["fxp0"]["units"][0]
    ...
    assert "families" in d["interfaces"]["lo0"]["units"][0]
    assert "inet" in d["interfaces"]["lo0"]["units"][0]["families"]
...
def test_switch_vlans():
    ...
    assert [101, 102, 103] == sd["cumulus__device_interfaces"]["lag_2000"][
        "tagged_vlans"
    ]
    # check bridge attrs
    assert 10000 == sd["cumulus__device_interfaces"]["bridge"]["mtu"]
    assert (
        sorted(["swp1", "lag_2000"])
        == sd["cumulus__device_interfaces"]["bridge"]["bridge_ports"]
    )
...
def test_switch_mgmt_interface():
    ...
    assert (
        ...
        == sd["cumulus__device_interfaces"]["eth0"]["description"]
    )
